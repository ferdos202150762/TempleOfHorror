...
if LOAD_PRETRAINED:
    agent_attacker = torch.load("attacker_agent_final.pth") # or a specific checkpoint
    agent_defender = torch.load("defender_agent_final.pth") # or a specific checkpoint
...
def test_agents(env, agents_dict, num_test_episodes=100, render_env=False):
    ...
    for episode in range(num_test_episodes):
        ...
        current_step = 0
        max_episode_steps_test = 200 # Or whatever is appropriate for your game

        while not done and current_step < max_episode_steps_test:
            if render_env:
            ...
            if round_type == "large": # Message stage
                ...
                if not done:
                     acting_player = env.get_current_player_role() # You'll need such a method in your env
            ...
            if done:
                ...
                if winner_info == "attacker": # Example
                    wins_attacker += 1
                elif winner_info == "defender":
                    wins_defender += 1
    ...
    print(f"Average Attacker Reward: {avg_attacker_reward:.2f}")
    print(f"Average Defender Reward: {avg_defender_reward:.2f}")
