...
class TempleCFR():
	...
	def cfr_iterations_external(self):
		...
		for t in tqdm(range(start_iteration, self.iterations + 1)):
			...
			for learning_player in [0,1,2]: # Players
				self.acting_player = 0 # Acting player is always zero
				...
				hand = config.compute_hand_from_labels(self.env_aux.static_player_hands[f'agent_{learning_player}'])

				cumulative_utility[learning_player] += self.external_cfr_message(f"P:{learning_player},R:{self.env_aux.player_role[f'agent_{learning_player}'][:-2]},C:{hand}GameInits->(P:{self.acting_player}", str(infoSet),  learning_player,  self.acting_player, t, probability_players)
	...
	def external_cfr(self, history, infoSet, learning_player, acting_player, t, probability_players):
		...
		if acting_player == learning_player:
			...
			hand_learn = config.compute_hand_from_labels(self.env_aux.static_player_hands[f'agent_{learning_player}'])
			prefix_hist_learn = f"P:{learning_player},R:{self.env_aux.player_role[f'agent_{learning_player}'][:-2]},C:{hand_learn}"
			hand_acting = config.compute_hand_from_labels(self.env_aux.static_player_hands[f'agent_{acting_player}'])
