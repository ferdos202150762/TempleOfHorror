...
### Env MARL Definition:
### 1. Agents: 3 1 defender and 2 attackers
### 2. Actions: 
###    - choose a player that open a card (3 cards per agent)
...
class TempleOfHorror():
    ...
    def sample_deck(self):
        ...
        for agent in self.agents:
            sampled_items = random.sample(aux, 3)
    ...
    def step(self, action):
        ...
        def open_card(agent_number):
            ...
            if sum(self.public_observation[f"agent_{agent_number}"]) == 3:
                for agent in self.agents: 
