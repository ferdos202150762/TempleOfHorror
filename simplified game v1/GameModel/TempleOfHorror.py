...
class TempleOfHorror():
    ...
    def sample_roles(self):
        ...
        for agent in self.agents:
            ...
            aux.remove(role)
        """
        self.player_role["agent_0"] = "defender_0"
        self.player_role["agent_1"] = "attacker_0"
        self.player_role["agent_2"] = "attacker_1"
    ...
    def sample_deck(self):
        ...
        for agent in self.agents:
            sampled_items = random.sample(aux, 2)
    ...
    def reward(self,agent, winner):
        ...
        if self.enc_player_role[agent] == winner:
            return 100 if self.enc_player_role[agent] == 1 else 50
        else:
            return -100 if self.enc_player_role[agent] == 1 else -50
    ...
    def step_message(self, action_message):
        ...
        self.message_space += action_message
    ...
    def step(self, action):
        ...
        def open_card(agent_number):
            ...
            if sum(self.observation_spaces[f"agent_{agent_number}"]) == 2:
                for agent in self.agents: 
